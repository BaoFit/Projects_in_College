...
void CCSocketClientDlg::OnChangeEdit1() 
{
	...
	// TODO: Add your control notification handler code here
	GetDlgItem(IDC_EDIT1)->GetWindowText(m_id);
}
...
void CCSocketClientDlg::OnButton1() 
{
	// TODO: Add your control notification handler code here
	//登陆
	CMySocket2 *cc=new CMySocket2();
	if(strcmp(m_id,"")==0)
		MessageBox("账号信息不能为空!");
	else if(strcmp(m_psw,"")==0)
		MessageBox("密码不能为空!");
	else
	{
		CString buf;
		...
		cc->sendValue(buf);
		cc->recvValue();
		if(strcmp(cc->recv_buf,"密码错误!")==0 || strcmp(cc->recv_buf,"账号不存在!")==0)
		{
			MessageBox(cc->recv_buf);
			cc->closeConnect();
		}
		else
		{
